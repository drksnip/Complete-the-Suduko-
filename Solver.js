document.addEventListener('DOMContentLoaded', function () {
    ...
    for (let row = 0; row < gridSize; row++) {
        ...
        // Wire restart button - non-refreshing reset
        const restartButton = document.getElementById('restart-btn');
        if (restartButton) {
            restartButton.addEventListener('click', function (e) {
                e.preventDefault(); // ensure no default navigation
                resetBoard();
            });
        }
    });
...
// Clear the board and UI without refreshing the page
function resetBoard() {
    const gridSize = 9;
    for (let row = 0; row < gridSize; row++) {
        for (let col = 0; col < gridSize; col++) {
            ...
            if (cell) {
                ...
                // If there are any additional visual marks, remove them here
